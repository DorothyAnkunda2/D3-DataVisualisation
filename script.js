...
const csvUrl = "water_quality_dataset.csv";
...
d3.csv(csvUrl, rowFormatter).then((data) => {
  ...
  const treeDataGroupedByDistricts = d3.groups(data, (d) => d.region);
  ...
  const donutChartUpdateFunction = donutChart(
    dataGroupedByDistrict,
    "#donughtchart"
  );
  const bubbleChartUpdateFunction = bubbleChart(
    dataGroupedByDistrict,
    "#bubblechart"
  );

  const groupedBarChartUpdateFunction = groupedBarChart(
    data,
    "#groupedbarchart"
  );
  ...
  function filterData(selectedRegion, selectedDistrict) {
    ...
    //   const previousRegion = d3.flatGroup(filteredData, (d) => d.region)[0][0];
...
function geoMap(data, selector) {
  ...
  const projection = d3.geoTransverseMercator().fitSize([width, height], data);
  ...
  // Example styling for the districts
  g.selectAll(".districts")
    .attr("fill", "#ccc")
    .attr("stroke", "#fff")
    .attr("stroke-width", 0.5);

  // Example to add labels to the districts
  ...
  // Example to add a title or tooltip to each district
  g.selectAll(".districts")
    .append("title")
    .text((d) => d.properties.name);

  // Apply margins to the map
  g.attr("transform", `translate(${margin.left},${margin.top})`);
}

// d3.json("topojson_uga_admbnda_adm2_ubos_20200824.json").then((topology) => {
//   const adminstrationUnits = topology.objects.uga_admbnda_adm2_ubos_20200824;
//   const districts = topojson.feature(topology, adminstrationUnits);
//   console.log(districts);

//   geoMap(districts, "#geomap");
// });
...
function donutChart(data, selector) {
  ...
  const margin = { top: 200, right: 20, bottom: 35, left: 200 };
